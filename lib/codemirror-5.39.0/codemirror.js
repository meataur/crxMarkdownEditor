...
function onDrop(e) {
  ...
  if (files && files.length && window.FileReader && window.File) {
    ...
    for (var i = 0; i < files.length; i++) {
      var fext = files[i].name.toLowerCase().split(".").pop();
    ...
    var n = files.length, text = Array(n);
    for (var i = 0; i < n; i++) {
      if (existmd) {
        var fext = files[i].name.toLowerCase().split(".").pop();
        if (fext == "md" || fext == "markdown") {
        ...
        var reader = new FileReader
        reader.onloadend = operation(cm, (function(idx, file) {
          return function(evt) {
            var fname = file.name;
            ...
            var hashing = document.querySelector('input[name="hashfunc"]:checked').value;
...
keyMap.basic = {
  ...
  "Tab": "defaultTab", "Shift-Tab": "indentLess", //"indentAuto",
